...
def parameter_rows(model) -> list[dict]:
    """Все тензоры параметров модели с отслеживанием связанных весов."""
    rows = []
    seen_ptrs = set()
    for name, param in model.named_parameters(remove_duplicate=False):
        ptr = param.data_ptr()
        is_tied = ptr in seen_ptrs
        seen_ptrs.add(ptr)
...
def forward_hooks(modules: dict) -> tuple[dict, list]:
    """Навесить forward-hooks на модули и вернуть словарь и список handles."""
...
def device_allocated_bytes(device: torch.device) -> int:
    """Пиковая память, выделенная на устройстве во время выполнения."""
    if device.type == "cuda":
...
def device_metric_source(device: torch.device) -> str:
    """Имя функции, которой снята пиковая память."""
    if device.type == "cuda":
...
class PeakMemory:
    """Замер пикового расхода памяти за время контекстного блока."""
    ...
    def result(self) -> dict:
        """Полная структура метрик памяти для отчёта."""
        rss_bytes, rss_source = peak_rss()
        is_accelerator = self.device.type in ("cuda", "mps")
        return {
            "peak_mb": self.used / (1024 ** 2),
            "peak_rss_mb": rss_bytes / (1024 ** 2),
            "metric": "пиковый allocated" if is_accelerator else "пиковая RSS",
